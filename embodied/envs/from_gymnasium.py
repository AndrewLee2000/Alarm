...
class FromGymnasium(embodied.Env):
  ...
  def step(self, action):
    ...
    if self._act_dict:
      act_flat = {k: v for k, v in action.items() if k != 'reset'}
      action = self._unflatten(act_flat)
    else:
  ...
  def _convert(self, space):
    ...
    if isinstance(space, gym.spaces.MultiDiscrete):
        low = np.zeros_like(space.nvec, dtype=np.int32)
        high = np.asarray(space.nvec, dtype=np.int32) - 1
        return elements.Space(np.int32, space.shape, low, high)
    return elements.Space(space.dtype, space.shape, space.low, space.high)