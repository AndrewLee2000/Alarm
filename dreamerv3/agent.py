...
class Agent(embodied.jax.Agent):
  ...
  def dream_loss(self, carry, obs, prevact, training):
    enc_carry, dyn_carry, dec_carry = carry
    reset = obs['is_first']
    B, T = reset.shape
    losses = {}
  ...
  def wake_report(self, carry, data): # FIXME
    ...
    # Train metrics
    _, (new_carry, entries, outs, mets) = self.wake_loss(
    ...
    if self.config.report_gradnorms:
      for key in self.scales:
        try:
          lossfn = lambda data, carry: self.wake_loss(
  ...
  def dream_report(self, carry, data): 
    ...
    # Train metrics
    _, (new_carry, entries, outs, mets) = self.dream_loss(
        carry, obs, prevact, training=False) 
    mets.update(mets)
    ...
    if self.config.report_gradnorms:
      for key in self.scales:
        try:
          lossfn = lambda data, carry: self.dream_loss(
              carry, obs, prevact, training=False)[1][2]['losses'][key].mean() 
          grad = nj.grad(lossfn, self.modules)(data, carry)[-1]
          metrics[f'gradnorm/{key}'] = optax.global_norm(grad)
        except KeyError:
