...
class Agent(embodied.Agent):
  ...
  def __init__(self, model, obs_space, act_space, config, jaxcfg):
    ...
    self._dream_train = transform.apply(
        nj.pure(self.model.dream_train), self.train_mesh,
        (dona_sharding, allo_sharding, tm, ts, ts), (tp, ts, ts, tm), ar,
        return_params=True, donate_params=True, first_outnums=(3,),
        **shared_kwargs)
    self._wake_train = transform.apply(
        nj.pure(self.model.wake_train), self.train_mesh,
        (dona_sharding, allo_sharding, tm, ts, ts), (tp, ts, ts, tm), ar,
        return_params=True, donate_params=True, first_outnums=(3,),
        **shared_kwargs)
    self._wake_report = transform.apply( 
        nj.pure(self.model.wake_report), self.train_mesh,
        (tp, tm, ts, ts), (ts, tm), ar,
        first_outnums=(1,), **shared_kwargs)
    self._dream_report = transform.apply( 
        nj.pure(self.model.dream_report), self.train_mesh,
        (tp, tm, ts, ts), (ts, tm), ar,
        first_outnums=(1,), **shared_kwargs)
    ...
    if self.jaxcfg.precompile:
      ...
      with self.train_mesh:
        self._compile_train()
        print('Dream_train cost analysis:')
        print(self._format_jit_stats(self._dream_train))
        print('Wake_train cost analysis:')
        print(self._format_jit_stats(self._wake_train))
        self._compile_report()
        print('Wake_report cost analysis:')
        print(self._format_jit_stats(self._wake_report))
        print('Dream_report cost analysis:')
        print(self._format_jit_stats(self._dream_report))
  ...
  @elements.timer.section('jaxagent_dream_train')
  def dream_train(self, carry, data):
    ...
    with self.train_lock:
      with elements.timer.section('jit_dream_train'):
        with jax.profiler.StepTraceAnnotation(
            'dream_train', step_num=int(self.n_updates)):
          self.params, carry, outs, mets = self._dream_train(
  ...
  @elements.timer.section('jaxagent_wake_train')
  def wake_train(self, carry, data):
    seed = data.pop('seed')
    assert sorted(data.keys()) == sorted(self.spaces.keys()), (
        sorted(data.keys()), sorted(self.spaces.keys()))
    allo = {k: v for k, v in self.params.items() if k in self.policy_keys}
    dona = {k: v for k, v in self.params.items() if k not in self.policy_keys}
    with self.train_lock:
      with elements.timer.section('jit_wake_train'):
        with jax.profiler.StepTraceAnnotation(
            'wake_train', step_num=int(self.n_updates)):
          self.params, carry, outs, mets = self._wake_train(
              dona, allo, seed, carry, data)
    self.n_updates.increment()
  ...
  def _init_params(self):
    ...
    params, params_sharding = transform.init(
        self.model.dream_train, self.train_mesh, # NOTE : model.dream_train과 model.wake_train은 같은 인자 (carry, data)를 받고 같은 형태 (params, carry, outs, mets)를 반환하므로, 둘 중 하나로 init하면 됨
  ...
  def _compile_train(self):
    ...
    allo = {k: v for k, v in self.params.items() if k in self.policy_keys}
    dona = {k: v for k, v in self.params.items() if k not in self.policy_keys}
    self._dream_train = self._dream_train.lower(dona, allo, seed, carry, data).compile()
    self._wake_train = self._wake_train.lower(dona, allo, seed, carry, data).compile()
  ...
  def _compile_report(self):
    ...
    self._wake_report = self._wake_report.lower(self.params, seed, carry, data).compile() 
    self._dream_report = self._dream_report.lower(self.params, seed, carry, data).compile() 
